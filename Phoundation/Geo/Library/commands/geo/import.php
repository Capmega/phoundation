...
if (!$argv['no_download'] and !$argv['no_import']) {
    Log::information(ts('Downloading geonames data'), 10);
...
if (!$argv['no_import']) {
    Log::information(ts('Importing geonames data'), 10);

    // Process the files
    Log::action(ts('Processing geonames files'), 10);
    ...
    Log::action(ts('Loading geonames files into temporary database'), 10);
    TODO FIX IMPORT
...
Log::action(ts('Importing geonames data into core database'), 10);
Import::import($argv['database']);

Log::success(ts('Finished importing all Geo data'), 10);
