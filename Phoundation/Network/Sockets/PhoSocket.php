...
class PhoSocket implements Stringable
{
     ...
     * (e.g. <code>127.0.0.1</code>).</p> <p>If the socket is of the AF_UNIX family, the address is the path of the
     * Unix-domain socket (e.g. <code>/tmp/my.sock</code>).</p>
    ...
    {
        unset(static::$source[$this->__toString()]);
     ...
     * is enabled and the socket is AF_INET6, or the pathname of a Unix domain socket, if the socket family is
     * AF_UNIX.</p>
     ...
     * </p><p><code>AF_INET6</code> - IPv6 Internet based protocols. TCP and UDP are common protocols of this
     * protocol family.</p><p><code>AF_UNIX</code> - Local communication protocol family. High efficiency and low
     * overhead make it a great form of IPC (Interprocess Communication).</p>
     ...
     * out-of-band data transmission mechanism may be supported. The TCP protocol is based on this socket type
     * .</p><p> <code>SOCK_DGRAM</code> - Supports datagrams (connectionless, unreliable messages of a fixed maximum
     * length). The UDP protocol is based on this socket type.</p><p><code>SOCK_SEQPACKET</code> - Provides a
     * sequenced, reliable, two-way connection-based data transmission path for datagrams of fixed maximum length; a
     * consumer is required to read an entire packet with each read call.</p><p><code>SOCK_RAW</code> - Provides raw
     * network protocol access. This special type of socket can be used to manually construct any type of protocol. A
     * common use for this socket type is to perform ICMP requests (like ping).</p><p><code>SOCK_RDM</code> -
     * Provides a reliable datagram layer that does not guarantee ordering. This is most likely not implemented on
     * your operating system.</p>
     * @param int $protocol <p>The protocol parameter sets the specific protocol within the specified domain to be
     * used when communicating on the returned socket. The proper value can be retrieved by name by using
     ...
     * datagram communication. The "ping" command (present in most modern operating systems) is an example
     * application of ICMP.</p><p>udp - The User Datagram Protocol is a connectionless, unreliable, protocol with
     * fixed record lengths. Due to these aspects, UDP requires a minimum amount of protocol overhead.</p><p>tcp -
     * The Transmission Control Protocol is a reliable, connection based, stream oriented, full duplex protocol. TCP
     * guarantees that all data packets will be received in the order in which they were sent. If any packet is
     * somehow lost during communication, TCP will automatically retransmit the packet until the destination host
     * acknowledges that packet. For reliability and performance reasons, the TCP implementation itself decides the
     ...
     * example, to retrieve options at the socket level, a level parameter of <code>SOL_SOCKET</code> would be used.
     * Other levels, such as <code>TCP</code>, can be used by specifying the protocol number of that level. Protocol
     ...
     * broadcast messages is supported. Returns int.</p><p><code>SO_REUSERADDR</code> - Reports whether local
     * addresses can be reused. Returns int.</p><p><code>SO_KEEPALIVE</code> - Reports whether connections are kept
     * active with periodic transmission of messages. If the connected socket fails to respond to these messages, the
     * connection is broken and processes writing to that socket are notified with a SIGPIPE signal. Returns int.</p>
     * <p> <code>SO_LINGER</code> - Reports whether the socket lingers on <code>close()</code> if data is
     * present. By default, when the socket is closed, it attempts to send all unsent data. In the case of a
     * connection-oriented socket, <code>close()</code> will wait for its peer to acknowledge the data. If
     * <code>l_onoff</code> is non-zero and <code>l_linger</code> is zero, all the unsent data will be discarded and
     * RST (reset) is sent to the peer in the case of a connection-oriented socket. On the other hand, if
     * <code>l_onoff</code> is non-zero and <code>l_linger</code> is non-zero, <code>close()</code> will block until
     * all the data is sent or the time specified in <code>l_linger</code> elapses. If the socket is non-blocking,
     * <code>close()</code> will fail and return an error. Returns an array with two keps: <code>l_onoff</code> and
     * <code>l_linger</code>.</p><p> <code>SO_OOBINLINE</code> - Reports whether the socket leaves out-of-band data
     * inline. Returns int.</p><p> <code>SO_SNDBUF</code> - Reports the size of the send buffer. Returns int.</p>
     * <p><code>SO_RCVBUF</code> - Reports the size of the receive buffer. Returns int.</p>
     * <p><code>SO_ERROR</code> - Reports information about error status and clears it. Returns int.</p>
     * <p><code>SO_TYPE</code> - Reports the socket type (e.g. <code>SOCK_STREAM</code>). Returns int.</p>
     * <p><code>SO_DONTROUTE</code> - Reports whether outgoing messages bypass the standard routing facilities.
     * Returns int.</p><p><code>SO_RCVLOWAT</code> - Reports the minimum number of bytes to process for socket input
     * operations. Returns int.</p><p><code>SO_RCVTIMEO</code> - Reports the timeout value for input operations.
     * Returns an array with two keys: <code>sec</code> which is the seconds part on the timeout value and
     * <code>usec</code> which is the microsecond part of the timeout value.</p><p> <code>SO_SNDTIMEO</code> -
     * Reports the timeout value specifying the amount of time that an output function blocks because flow control
     * prevents data from being sent. Returns an array with two keys: <code>sec</code> which is the seconds part on
     * the timeout value and <code>usec</code> which is the microsecond part of the timeout value.</p>
     * <p><code>SO_SNDLOWAT</code> - Reports the minimum number of bytes to process for socket output operations.
     * Returns int.</p><p><code>TCP_NODELAY</code> - Reports whether the Nagle TCP algorithm is disabled. Returns
     * int.</p><p><code>IP_MULTICAST_IF</code> - The outgoing interface for IPv4 multicast packets. Returns the index
     * of the interface (int).</p><p><code>IPV6_MULTICAST_IF</code> - The outgoing interface for IPv6 multicast
     * packets. Returns the same thing as <code>IP_MULTICAST_IF</code>.</p><p><code>IP_MULTICAST_LOOP</code> - The
     * multicast loopback policy for IPv4 packets, which determines whether multicast packets sent by this socket
     * also reach receivers in the same host that have joined the same multicast group on the outgoing interface used
     * by this socket. This is the case by default. Returns int.</p><p><code>IPV6_MULTICAST_LOOP</code> - Analogous
     * to <code>IP_MULTICAST_LOOP</code>, but for IPv6. Returns int.</p><p><code>IP_MULTICAST_TTL</code> - The
    ...
    public function getOption(int $level, int $optname): mixed
    {
        ...
        if (!$return) {
            throw SocketException::new(tr('Failed to get socket option ":optname" at leve; :"level"', [
                ':optname' => $optname,
                ':level'   => $level
            ]))
            ->setCode(socket_last_error($this->resource))
     ...
     *
     * @param string $address <p>If the given socket is of type <code>AF_INET</code> or <code>AF_INET6</code>,
     * <code>getSockName()</code> will return the local IP address in appropriate notation (e.g. <code>127.0.0.1</code>
     * or <code>fe80::1</code>) in the address parameter and, if the optional port parameter is present, also the
     * associated port.</p><p>If the given socket is of type <code>AF_UNIX</code>, <code>getSockName()</code> will
     * return the Unix filesystem path (e.g. <code>/var/run/daemon.sock</cod>) in the address parameter.</p>
     ...
     *
     * @param int $backlog <p>A maximum of backlog incoming connections will be queued for processing. If a
     ...
     * retries may succeed.</p><p> <b>Note:</b> The maximum number passed to the backlog parameter highly depends on
     * the underlying platform. On Linux, it is silently truncated to <code>SOMAXCONN</code>. On win32, if passed
     * <code>SOMAXCONN</code>, the underlying service provider responsible for the socket will set the backlog to a
     ...
     * (Default) - use the system <code>recv()</code> function. Safe for reading binary data.</li><li>
     ...
     * <li><code>MSG_PEEK</code> - Receive data from the beginning of the receive queue without removing it
     * from the queue.</li><li> <code>MSG_WAITALL</code> - Block until at least length are received. However, if a
     * signal is caught or the remote host disconnects, the function may return less data.</li>
     * <li><code>MSG_DONTWAIT</code> - With this flag set, the function returns even if it would normally have blocked.
     * </li></ul></p>
     *
     ...
     *
     */
    public static function select(array &$read, array &$write, array &$except, ?int $timeoutSeconds, int $timeoutMilliseconds = 0): int {
        ...
        $read   = $readSockets   ? static::mapRawSocketToClass($readSockets)   : [];
        $write  = $writeSockets  ? static::mapRawSocketToClass($writeSockets)  : [];
        $except = $exceptSockets ? static::mapRawSocketToClass($exceptSockets) : [];
    ...
    protected static function mapClassToRawSocket(array $sockets): array
    {
        ...
        foreach ($sockets as $socket) {
            if ($socket instanceof PhoSocket) {
                if ($socket->resource) {
    ...
    protected static function mapRawSocketToClass(array $sockets): array
    {
        ...
        foreach ($sockets as $rawSocket) {
            $socketHash = spl_object_hash($rawSocket);

            if (empty(static::$source[$socketHash])) {
                throw SocketException::new(tr('Failed to map socket with hash ":hash"', [
                    ':hash' => $socketHash
                ]));
            }

            $return[$socketHash] = static::$source[$socketHash];
        }
     ...
     * @param int $flags <p>The value of flags can be any combination of the following flags, joined with the binary
     * OR (<code>|</code>) operator.<ul><li><code>MSG_OOB</code> - Send OOB (out-of-band) data.</li><li>
     * <code>MSG_EOR</code> - Indicate a record mark. The sent data completes the record.</li>
