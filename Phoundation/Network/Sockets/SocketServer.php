...
use Phoundation\Network\Sockets\Exception\ServerException;
...
class SocketServer
{
     ...
     *
     *
     * @param string $address The address to listen on. Must be a valid IPv4, IPv6, or Unix socket address
     * @param int $port
    ...
    protected function loopOnce(): bool
    {
        if (empty($this->masterSocket)) {
            throw new ServerException(tr('Socket must be started before running server loop'));
        }
