...
class Plugins extends Project
{
    ...
    public function detectLocation(?string $location = null): string
    {
        ...
        if ($location) {
            ...
            $this->_restrictions = PhoRestrictions::new(dirname($directory));
            ...
            Log::success(ts('Using Phoundation plugins installation in specified directory ":directory"', [
                ':directory' => $directory,
            ]));

            $this->_directory = new PhoDirectory($directory);
        ...
        foreach ($directories as $directory) {
            ...
            foreach ($names as $name) {
                ...
                $this->_restrictions = PhoRestrictions::new(dirname($test_path));
                ...
                Log::success(ts('Found Phoundation plugins installation in ":directory"', [
                    ':directory' => $test_path,
                ]));

                $this->_directory = new PhoDirectory($test_path);
    ...
    {
        return PhoDirectory::new($directory . 'Plugins', $this->_restrictions)->isReadable();
    }
    ...
    public function switchBranch(?string $branch = null): static
    {
        if ($branch === null) {
            ...
            Log::action(ts('Switching Phoundation plugins back to branch ":branch"', [
                ':branch' => $branch,
            ]), 3);

            $this->_git->selectBranch($this->branch);
            $this->branch = null;

        } else {
            // Select the previous branch and reset it
            Log::action(ts('Switching Phoundation plugins to branch ":branch"', [
                ':branch' => $branch,
            ]), 4);

            // Select the new branch and store the previous
            $this->branch = $this->_git->getSelectedBranch();
    ...
    protected function selectPluginsBranch(?string $branch): static
    {
        ...
        $this->phoundation_branch = $this->_git->getSelectedBranch();
        ...
        if ($branch !== $this->phoundation_branch) {
            ...
            Log::action(ts('Switching Phoundation plugins branch to requested branch ":requested"', [
                ':requested' => $branch,
            ]), 5);
    ...
    public function patch(?string $branch, ?string $message, ?bool $sign = null, bool $checkout = true, bool $update = true): void
    {
        ...
        Log::action(ts('Patching branch ":branch" on your local Phoundation plugins repository from this project', [
            ':branch' => $branch,
        ]));
        ...
        try {
            ...
            if ($non_phoundation_stash) {
                ...
                Log::warning(ts('Un-stashing non phoundation plugins'));
    ...
    public function stashNonPhoundationPlugins(): bool
    {
        ...
        if ($non_phoundation_plugins) {
            Log::warning(ts('Stashing non phoundation plugins ":plugins"', [
    ...
    public function getNonPhoundationPlugins(): array
    {
        ...
        foreach ($plugins as $plugin) {
            ...
            if (PhoDirectory::new(DIRECTORY_ROOT . 'Plugins/' . $plugin)->containFiles()) {
                ...
                Log::warning(ts('Ignoring plugin ":plugin" because it contains no files', [
                    ':plugin' => $plugin,
                ]));
            }
        }
    ...
    {
        return PhoDirectory::new($this->_directory . 'Plugins/', $this->_directory->getRestrictionsObject())
                           ->scan()
                           ->forEachField(function (&$value, $key) {
                              $value = Strings::ensureEndsNotWith($value, '/');
                          });
...
throw new UnderConstructionException(tr('Plugins::updateTo() is under construction, not sure what it is supposed to do'));
        ...
        foreach ($this->phoundation_files as $directory) {
            $directory = $this->_git->getDirectoryObject() . $directory;

            // Find local Phoundation changes and filter Phoundation changes only
            $changed_files = $this->_git->getStatusFilesObject($directory);
            ...
            $changed_files->applyPatch($this->_directory);
