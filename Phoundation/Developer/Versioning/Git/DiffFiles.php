...
use Phoundation\Developer\Versioning\Git\Exception\GitDiffException;
...
class DiffFiles extends PhoFilesCore
{
    ...
    public function scanChanges(): static
    {
        ...
        foreach ($files as $file) {
            if (str_starts_with(strtolower($file), 'warning:')) {
                throw GitDiffException::new(tr('Failed to fetch git diff for file ":file", it failed with ":error"', [
            ...
            try {
                ...
                throw GitUnknownDiffException::new(tr('Unknown git diff ":diff" encountered for file ":file"', [
                    ':file'   => $file,
                    ':diff' => $diff,
                ]), $e)->setData([
                    'file'   => $file,
