...
class Zip extends Command implements ZipInterface
{
    ...
    public function unzip(PhoDirectoryInterface $target): PhoDirectoryInterface
    {
        try {
            ...
            // The command unzip failed, most of the time either $file does not exist, or we do not have access
    ...
    public function zip(?PhoFileInterface $target = null): PhoFileInterface
    {
        try {
            if (!$target) {
                $parent = $this->source_path->getParentDirectoryObject();
            ...
            // The command zip failed, most of the time either $file does not exist, or we do not have access
