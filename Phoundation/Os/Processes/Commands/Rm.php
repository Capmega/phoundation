...
class Rm extends Command
{
     ...
     *                             deleted
     * @param bool   $recurse_up   If set to true, all directories above will be deleted as well IF they are empty
    ...
    public function delete(string $file, bool $recurse_down = true, bool $recurse_up = false, int $timeout = 10): void
    {
        try {
            ...
            if ($recurse_up) {
                ...
                while ($empty) {
                    $file  = dirname($file);
                    $empty = PhoDirectory::new($file, $this->_restrictions)
                                         ->isEmpty();
                    if ($empty) {
            ...
            // The command rm failed, most of the time either $file does not exist, or we do not have access to change the mode
