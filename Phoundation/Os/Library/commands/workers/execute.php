...
$argv = ArgvValidator::new()
                     ...
                     ->select('-a,--arguments', true)->isOptional()->hasMaxCharacters(8192)->sanitizeSearchReplace([' ' => ','])->sanitizeForceArray()->forEachField()->isString()
                     ->select('-t,--worker-timeout', true)->isOptional(30)->isPositive()
                     ->select('-s,--cycle-sleep', true)->isOptional(1_000_000)->isPositive()
                     ->select('-v,--values', true)->isOptional()->sanitizeForceArray()
                     ->select('-q,--query', true)->isOptional()->isString()
...
//      ->setQuery($argv['query'])
//      ->setExecute($argv['execute'])
      ->setCycles($argv['cycles'])
      ->setValues($argv['values'])
