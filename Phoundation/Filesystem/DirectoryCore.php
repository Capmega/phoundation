 ...
 */
class DirectoryCore extends PathCore implements DirectoryInterface
{
    /**
    ...
    public static function removeTemporary(): void
    {
        Core::ExecuteNotInTestMode(function () {
            ...
            if (static::$temp_directory_private) {
                File::new(static::$temp_directory_private, Restrictions::new(DIRECTORY_TMP, true))
                    ->delete();
                $action = true;
            }
            if (static::$temp_directory_public) {
                File::new(static::$temp_directory_public, Restrictions::new(DIRECTORY_PUBTMP, true))
                    ->delete();
                $action = true;
            }
    ...
    public function createTarget(?bool $single = null, int $length = 0): string
    {
        ...
        $this->path = Strings::unslash(Directory::new($this->path, $this->restrictions)
                                                ->ensure()
                                                ->getPath());
        if ($single) {
        ...
        return Strings::slash(Directory::new($this->path, $this->restrictions)
                                       ->ensure()
                                       ->getPath());
    }
    ...
    public function listTree(array|string|null $filters = null, bool $recursive = true): array
    {
        ...
        while (($filename = readdir($fh)) !== false) {
            ...
            if (is_dir($file) and $recursive) {
                $return = array_merge($return, Directory::new($file, $this->restrictions)
                                                        ->listTree());
        ...
        $this->execute()
             ...
             ->onFiles(function (string $file) use (&$return) {
                 try {
                     ...
                     // Add file statistics
                     $return['files_statistics'][$file] = File::new($file, $this->restrictions)
                                                              ->getPhpStatistics();
    ...
    {
        return File::new(Tar::new($this->restrictions)
                            ->tar($this->path), $this->restrictions);
    }
        ...
        if ($mounted and !$not_mounted) {
            ...
            if ($sources) {
                ...
                foreach ($sources as $source) {
                    if (
                        $mount['source'] == Directory::new($source)
                                                     ->getPath()
                    ) {
                        return true;
                    }
     ...
     * @return static
     * @example:
     * File::new($source)->copy($target, $restrictions, function ($notification_code, $severity, $message,
