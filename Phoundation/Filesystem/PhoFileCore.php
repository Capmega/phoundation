...
//    }
    ...
    public function ensureFile($mode = null, $pattern_mode = null): void
    {
        ...
        $this->_restrictions->check($directory, true);

        PhoDirectory::new(dirname($this->source), $this->_restrictions)->ensure($pattern_mode);

        if (!file_exists($this->source)) {
            // Create the file
            PhoDirectory::new(dirname($this->source), $this->_restrictions)
                     ->execute()
...
//    }
     ...
     * do the same as 0000
     */
    public function copyTree(string $destination, ?array $search = null, ?array $replace = null, array|string|null $extensions = null, mixed $mode = true, bool $novalidate = false): string
    {
        throw new UnderConstructionException('$this->copyTree() is under construction');
        // Check filesystem restrictions
        $this->_restrictions->check($source, false);
        $this->_restrictions->check($destination, true);
...
// :TODO: Check if dirname($this->file) here is correct? It somehow does not make sense
        ...
        if (is_dir($source)) {
            ...
            foreach (scandir($source) as $this->source) {
                ...
                if (is_null($mode)) {
                    $this->filemode = config()->get('filesystem.modes.defaults.directories', $mode ?? 0640);
                ...
                if (is_dir($source . $this->source)) {
                    ...
                    if (file_exists($destination . $this->source)) {
                        ...
                        if (!is_dir($destination . $this->source)) {
                            ...
                            file_delete($destination . $this->source, $this->_restrictions);
                        }
                    }
    ...
    public function backup(string $pattern = '~date', bool $move = false): static
    {
        ...
        return new static($target, $this->_restrictions);
    }
    ...
    public function ensureReadable(?int $mode = null): static
    {
        ...
        $mode = config()->get('filesystem.mode.default.file', $mode ?? 0440);
    ...
    {
        // Get configuration. We need file and directory default modes
        $mode = config()->get('filesystem.mode.default.file', $mode ?? 0640);
    ...
    {
        $file_sha = Sha256::new($this->_restrictions)->sha256($this->source);
    ...
    public function untar(): PhoPathInterface
    {
        ...
        return PhoPath::new(dirname($this->source), $this->_restrictions);
    }
    ...
    {
        return Gzip::new($this->_restrictions)->gzip($this);
    }
    ...
    {
        return Gzip::new($this->_restrictions)->gunzip($this);
    }
    ...
    public function replace(array $replaces, ?PhoFileInterface $target = null, bool $regex = false): static
    {
        ...
        // Check filesystem restrictions and if file exists
        $this->_restrictions->check($this->source, false);
        $target->_restrictions->check($target, true);
