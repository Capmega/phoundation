 ...
 * X$PATHS$         Restrict access to the specified dot-comma separated $PATHS$ list. $PATHS is optional and defaults to DIRECTORY_ROOT/web, DIRECTORY_ROOT/data/downloads
 * Y                Execute the resolved page as a system page. Requires the resolved page to be an HTTP code compatible integer number
 *
 ...
 * @package route
 * @see \Phoundation\Web\Routing\Route::404()
 * @see \Phoundation\Web\Routing\Route::mappedDomain()
 * @table: `route`
...
Route::getParametersObject()
     ...
     ->add(RoutingParameters::new('/^\w{2}\/api/') // Routing parameters for API requests
                            ->setRootUrl(':PROTOCOL://:DOMAIN/:LANGUAGE/'))
...
Route::try('/^(sitemap.xml|robots.txt)?$/'                                     , '$1'                   , '');          // Pass along robots.txt or the sitemap.xml
