...
class MetaList
{
    ...
    public function getHtmlDataTable(array|string|null $columns = null): HtmlDataTableInterface
    {
        ...
        foreach ($source as &$row) {
            if ($row['created_by']) {
                $row['user'] = '<a href="' . Url::getWww('profiles/profile+' . $row['created_by'] . '.html') . '">' . Html::safe($row['user']) . '</a>';
            }
            ...
            if (Url::isValid($row['source'])) {
                $row['source'] = '<a href = "' . Html::safe($row['source']) . '">' . Html::safe($row['source']) . '</a>';
            }
            ...
            if (isset_get($row['data']['to'])) {
                ...
                $row['data'] = str_replace(PHP_EOL, '<br>', Html::safe($row['data']));
