...
function tr($text, $replace = null, $verify = true)
{
    ...
    try {
        if ($replace) {
            ...
            if (empty($_CONFIG['production']) and $verify) {
                if ($count != count($replace)) {
                    ...
                    throw new OutOfBoundsException('tr(): Not all specified keywords were found in text', 'not-exists');
                }
         ...
         */
        throw new OutOfBoundsException('tr(): Failed with text "' . str_log($text) . '". Very likely issue with $replace not containing all keywords, or one of the $replace values is non-scalar', $e);
    }
}
...
function load_external($files)
{
    try {
        ...
        throw new OutOfBoundsException(tr('load_external(): Failed'), $e);
    }
}
...
function load_libs($libraries)
{
    ...
    try {
        ...
        foreach (array_force($libraries) as $library) {
            if (!$library) {
                notify(new OutOfBoundsException('load_libs(): Empty library specified', 'warning/not-specified'));
                continue;
            }
            ...
            if ($core->register['ready'] and str_exists('http,strings,array,sql,mb,meta,file,json', $library)) {
                 ...
                 * load them again
                 */
                notify(new OutOfBoundsException(tr('load_libs(): Library ":library" was already loaded during system startup, not loading again', array(':library' => $library)), 'warning/already-done'));
                continue;
            }
        ...
        if (isset($library)) {
            $e = new OutOfBoundsException(tr('load_libs(): Failed to load library ":library"', array(':library' => $library)), $e);

        } else {
            $e = new OutOfBoundsException(tr('load_libs(): Failed to load libraries ":libraries"', array(':libraries' => $libraries)), $e);
        }
        ...
        if (empty($library)) {
            throw new OutOfBoundsException(tr('load_libs(): Failed to load one or more of libraries ":libraries"', array(':libraries' => $libraries)), $e);
        }

        if (!file_exists($libs . $library . '.php')) {
            throw new OutOfBoundsException(tr('load_libs(): Failed to load library ":library", the library does not exist', array(':library' => $library)), $e);
        }

        throw new OutOfBoundsException(tr('load_libs(): Failed to load library ":library" from the requested libraries list ":libraries"', array(':libraries' => $libraries, ':library' => $library)), $e);
    }
}
...
function load_content($file, $replace = false, $language = null, $autocreate = null, $validate = true)
{
    ...
    try {
        ...
        if ($realfile) {
            ...
            if ($validate and preg_match('/###[a-z]+?###/i', $retval, $matches)) {
                ...
                if (!$_CONFIG['production']) {
                    throw new OutOfBoundsException(tr('load_content(): Missing markers ":markers" for content file ":file"', array(':markers' => $matches, ':file' => $realfile)), 'missing-markers');
                }
            }
        ...
        if (!$autocreate) {
            throw new OutOfBoundsException(tr('load_content(): Specified file ":file" does not exist for language ":language"', array(':file' => $file, ':language' => $language)), 'not-exists');
        }
        ...
        throw new OutOfBoundsException(tr('load_content(): Failed for file ":file"', array(':file' => $file)), $e);
    }
}
...
function in_source($source, $key, $return = true)
{
    try {
        if (!is_array($source)) {
            throw new OutOfBoundsException(tr('in_source(): Specified source ":source" should be an array', array(':source' => $source)), 'invalid');
        }
        ...
        throw new OutOfBoundsException('in_source(): Failed', $e);
    }
}
...
function is_natural($number, $start = 1)
{
    try {
        ...
        throw new OutOfBoundsException('is_natural(): Failed', $e);
    }
}
...
function is_version($version)
{
    try {
        ...
        throw new OutOfBoundsException('is_version(): Failed', $e);
    }
}
...
function is_new($entry)
{
    try {
        if (!is_array($entry)) {
            throw new OutOfBoundsException(tr('is_new(): Specified entry is not an array'), 'invalid');
        }
        ...
        throw new OutOfBoundsException('is_new(): Failed', $e);
    }
}
...
function force_natural($number, $default = 1, $start = 1)
{
    try {
        ...
        throw new OutOfBoundsException('force_natural(): Failed', $e);
    }
}
...
function disconnect()
{
    try {
        ...
        throw new OutOfBoundsException(tr('disconnect(): Failed'), $e);
    }
}
...
function get_boolean($value)
{
    try {
        switch (strtolower($value)) {
            ...
            default:
                throw new OutOfBoundsException(tr('get_boolean(): Unknown value ":value"', array(':value' => $value)), 'unknown');
        }

    } catch (Exception $e) {
        throw new OutOfBoundsException(tr('get_boolean(): Failed'), $e);
    }
}
...
function str_cut($source, $start, $stop)
{
    try {
        ...
        throw new OutOfBoundsException(tr('str_cut(): Failed'), $e);
    }
}
...
function str_exists($haystack, $needle)
{
    try {
        ...
        throw new OutOfBoundsException(tr('str_exists(): Failed'), $e);
    }
}
...
function str_clean($source, $utf8 = true)
{
    try {
        if (!is_scalar($source)) {
            if (!is_null($source)) {
                throw new OutOfBoundsException(tr('str_clean(): Specified source ":source" from ":location" should be datatype "string" but has datatype ":datatype"', array(':source' => $source, ':datatype' => gettype($source), ':location' => current_file(1) . '@' . current_line(1))), 'invalid');
            }
        }
...
//            $source = preg_replace('/\s|\/|\?|&+/u', $replace, $source);
        ...
        throw new OutOfBoundsException(tr('str_clean(): Failed with string ":string"', array(':string' => $source)), $e);
    }
...
function str_from($source, $needle, $more = 0, $require = false)
{
    try {
        if (!$needle) {
            throw new OutOfBoundsException('str_from(): No needle specified', 'not-specified');
        }
        ...
        throw new OutOfBoundsException(tr('str_from(): Failed for string ":string"', array(':string' => $source)), $e);
    }
}
...
function str_until($source, $needle, $more = 0, $start = 0, $require = false)
{
    try {
        if (!$needle) {
            throw new OutOfBoundsException('str_until(): No needle specified', 'not-specified');
        }
        ...
        throw new OutOfBoundsException(tr('str_until(): Failed for string ":string"', array(':string' => $source)), $e);
    }
}
...
function str_rfrom($source, $needle, $more = 0)
{
    try {
        if (!$needle) {
            throw new OutOfBoundsException('str_rfrom(): No needle specified', 'not-specified');
        }
        ...
        throw new OutOfBoundsException(tr('str_rfrom(): Failed for string ":string"', array(':string' => $source)), $e);
    }
}
...
function str_runtil($source, $needle, $more = 0, $start = 0)
{
    try {
        if (!$needle) {
            throw new OutOfBoundsException('str_runtil(): No needle specified', 'not-specified');
        }
        ...
        throw new OutOfBoundsException(tr('str_runtil(): Failed for string ":string"', array(':string' => $source)), $e);
    }
}
...
function str_starts($source, $string)
{
    try {
        ...
        throw new OutOfBoundsException(tr('str_starts(): Failed for ":source"', array(':source' => $source)), $e);
    }
}
...
function str_starts_not($source, $string)
{
    try {
        ...
        throw new OutOfBoundsException(tr('str_starts_not(): Failed for ":source"', array(':source' => $source)), $e);
    }
}
...
function str_ends($source, $string)
{
    try {
        ...
        throw new OutOfBoundsException('str_ends(): Failed', $e);
    }
}
...
function str_ends_not($source, $strings, $loop = true)
{
    try {
        ...
        throw new OutOfBoundsException('str_ends_not(): Failed', $e);
    }
}
...
function slash($string)
{
    try {
        ...
        throw new OutOfBoundsException('slash(): Failed', $e);
    }
}
...
function unslash($string, $loop = true)
{
    try {
        ...
        throw new OutOfBoundsException('unslash(): Failed', $e);
    }
}
...
function str_nodouble($source, $replace = '\1', $character = null, $case_insensitive = true)
{
    try {
        ...
        throw new OutOfBoundsException('str_nodouble(): Failed', $e);
    }
}
...
function str_truncate($source, $length, $fill = ' ... ', $method = 'right', $on_word = false)
{
    try {
        if (!$length or ($length < (mb_strlen($fill) + 1))) {
            throw new OutOfBoundsException('str_truncate(): No length or insufficient length specified. You must specify a length of minimal $fill length + 1', 'invalid');
        }
        ...
        switch ($method) {
            ...
            default:
                throw new OutOfBoundsException(tr('str_truncate(): Unknown method ":method" specified, please use "left", "center", or "right" or undefined which will default to "right"', array(':method' => $method)), 'unknown');
        }

    } catch (Exception $e) {
        throw new OutOfBoundsException(tr('str_truncate(): Failed for ":source"', array(':source' => $source)), $e);
    }
}
...
function str_log($source, $truncate = 8187, $separator = ', ')
{
    try {
        ...
        throw new OutOfBoundsException('str_log(): Failed', $e);
    }
}
...
function array_first($source)
{
    try {
        ...
        throw new OutOfBoundsException('array_first(): Failed', $e);
    }
}
...
function array_last($source)
{
    try {
        ...
        throw new OutOfBoundsException('array_last(): Failed', $e);
    }
}
...
function array_ensure(&$source, $keys = '', $default_value = null, $trim_existing = false)
{
    try {
        if (!$source) {
            ...
            if (is_object($source)) {
                throw new OutOfBoundsException(tr('array_ensure(): Specified source is not an array but an object of the class ":class"', array(':class' => get_class($source))), 'invalid');
            }

            throw new OutOfBoundsException(tr('array_ensure(): Specified source is not an array but a ":type"', array(':type' => gettype($source))), 'invalid');
        }
        ...
        throw new OutOfBoundsException('array_ensure(): Failed', $e);
    }
}
...
function array_force($source, $separator = ',')
{
    try {
        ...
        throw new OutOfBoundsException('array_force(): Failed', $e);
    }
}
...
class Colors
{
    ...
    public function __construct()
    {
        try {
            ...
            throw new OutOfBoundsException(tr('Color::__construct(): Failed'), $e);
        }
    }
...
function force_datatype($source)
{
    try {
        ...
        throw new OutOfBoundsException('force_datatype(): Failed', $e);
    }
}
...
function get_true_false($value, $default)
{
    try {
        switch ($value) {
            ...
            default:
                throw new OutOfBoundsException(tr('get_true_false(): Please specify y / yes or n / no, or nothing for the default value.'), 'warning');
        }

    } catch (Exception $e) {
        throw new OutOfBoundsException(tr('get_true_false(): Failed'), $e);
    }
}
...
function get_yes_no($value)
{
    try {
        ...
        throw new OutOfBoundsException(tr('get_yes_no(): Please specify true or false'), 'warning');

    } catch (Exception $e) {
        throw new OutOfBoundsException(tr('get_yes_no(): Failed'), $e);
    }
}
