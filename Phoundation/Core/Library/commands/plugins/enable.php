 ...
 *
 * This command allows you to enable registered plugins
 *
...
CliDocumentation::setAutoComplete([
    'positions' => [
        0 => [
            'word'   => 'SELECT `name` FROM `core_plugins` WHERE `name` LIKE :word AND `status` = "disabled" LIMIT ' . Limit::getShellAutoCompletion(),
            'noword' => 'SELECT `name` FROM `core_plugins` WHERE                       `status` = "disabled" LIMIT ' . Limit::getShellAutoCompletion(),
        ],
    ],
]);
...
$argv = ArgvValidator::new()
                     ->selectAll('plugins')->isOptional()->xorArgument('all', ALL)->sanitizeForceArray()->forEachField()->isName()
                     ->validate();
...
if (ALL) {
    ...
    $plugin = Plugins::new()->load()->forEachField(function ($plugin, $plugin_name) {
        // Enable plugin
        Plugin::new()->load($plugin_name)->enable();
    });
