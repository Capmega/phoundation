...
CliDocumentation::setAutoComplete([
    'positions' => [
        0 => [
            'word'   => 'SELECT `name` FROM `core_plugins` WHERE `name` LIKE :word AND (`status` IS NULL OR `status` NOT LIKE "deleted%") LIMIT ' . Limit::getShellAutoCompletion(),
            'noword' => 'SELECT `name` FROM `core_plugins` WHERE                       (`status` IS NULL OR `status` NOT LIKE "deleted%") LIMIT ' . Limit::getShellAutoCompletion(),
        ],
    ],
    'arguments' => [
        '-p,--priority' => true,
