...
if ($argv['library']) {
    ...
    foreach ($types as $type) {
        if (empty($argv['type']) or ($argv['type'] === $type)) {
            $statistics[$type]['total_statistics']['size'] = Numbers::getHumanReadableBytes(array_get($statistics, $type['total_statistics']['size'], 0));

            Log::cli(' ');
            Log::cli(CliColor::apply(tr('Statistics for ":type" libraries:', [':type' => $type]), 'information'));
            Cli::displayForm(array_get($statistics, $type['total_statistics']), 0);
            Log::cli(' ');
        }
    }
}
