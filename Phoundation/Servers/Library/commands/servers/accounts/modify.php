...
./pho servers accounts modify NAME -u phoundation -k KEYFILE -d "This is a test account"');
...
if ($argv['ssh_key_file']) {
    PhoFile::new($argv['ssh_key_file'], $argv['ssh_key_file'])->ensureReadable();
...
// Done!
Log::success(ts('Modified SSH account ":account"', [':account' => $account->getName()]), 10);
