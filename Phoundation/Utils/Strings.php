...
//    return mysqli_escape_string(trim(decode_entities(mb_strip_tags($str))));
     ...
     * @throws OutOfBoundsException
     */
    public static function getRandom(int $length = 8, bool $unique = false, Stringable|string $source = 'alphanumeric'): string
    {
        $source = (string) $source;
        ...
        for ($i = 0; $i < $length; $i++) {
            $char = $source[Numbers::getRandomInt(0, $char_length - 1)];
     ...
     * @param Stringable|string|float|int|null $source
     * @param int                              $size
     * @param string                           $fill_string
     * @param bool                             $prefix
     *
     * @return string
     */
    public static function size(Stringable|string|float|int|null $source, int $size, string $fill_string = ' ', bool $prefix = false): string
    {
        if ($size < 0) {
        ...
        $blocks      = floor($fill_size / $block_size);
        $extra       = $fill_size - ($block_size * $blocks);
        $fill_string = str_repeat($fill_string, (int) $blocks) . substr($fill_string, 0, (int) $extra);

        if ($prefix) {
            return $fill_string . $source;
        }

        return $source . $fill_string;
    }
    ...
    public static function getStringValue(mixed $source, ?string $key): ?string
    {
        if ($key) {
            try {
                ...
                throw new OutOfBoundsException(tr('Specified key ":key" does not exist in the given value ":value"', [
                    ':key'   => $key,
                    ':value' => $source,
                ]), $e);
            }
        ...
        if (is_array($source) or ($source instanceof EntryInterface)) {
            throw OutOfBoundsException::new(tr('Cannot extract string value from specified source array or EntryInterface object, no key specified'))
                                      ->addData([
                                          'source' => $source,
                                      ]);
        }
