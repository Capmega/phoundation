...
class SignInKey extends DataEntry implements SignInKeyInterface
{
    ...
    public function signKeyAllowsUrl(Stringable|string $url, string $target): bool
    {
        ...
        if (!str_starts_with($target, (DIRECTORY_WEB . 'pages/system/'))) {
            // For this URL, we are trying to display a system page instead. Allow too
            return true;
        }
    ...
    protected function setDefinitionsObject(DefinitionsInterface $_definitions): static
    {
        $_definitions->add(DefinitionFactory::newUsersId())
                      ...
                      ->add(DefinitionFactory::newDate('valid_until')
                                           ->setLabel(tr('Valid until')))

                    ->add(DefinitionFactory::newBoolean('allow_navigation')
