...
* @return static
    ...
    public function delete(?string $comments = null, bool $auto_save = true): static
    {
        if ($this->getId(false)) {
            ...
            sql()->query('UPDATE `accounts_users_roles` SET status = "' . static::generateStatus('deleted') . '" WHERE `roles_id` = :roles_id', [
                ':roles_id' => $this->getId(),
            ]);

            sql()->query('UPDATE `accounts_roles_rights` SET status = "' . static::generateStatus('deleted') . '" WHERE `roles_id` = :roles_id', [
                ':roles_id' => $this->getId(),
            ]);
        }
