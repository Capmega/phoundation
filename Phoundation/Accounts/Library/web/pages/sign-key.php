...
try {
    ...
    Request::executeSystem(401, tr('This link has already been used, please request a new link'));

} catch (SignInKeyDifferentUserException $e) {
    // Show authentication failed but add a message
    Request::executeSystem(401, tr('This link is for a different user'));

} catch (SignInKeyExpiredException $e) {
    // Show authentication failed but add a message
    Request::executeSystem(401, tr('This link expired, please request a new link'));

} catch (SignInKeyStatusException $e) {
    // Show authentication failed but add a message
    Request::executeSystem(401, tr('This link is no longer valid, please request a new link'));
