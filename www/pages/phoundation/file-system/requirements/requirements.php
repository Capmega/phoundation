...
switch ($filters->getSourceKey('entry_status')) {
    ...
    default:
        $builder->addWhere('`filesystem_requirements`.`status` = :status', [':status' => $filters->getSourceKey('entry_status')]);
}

// Build SQL requirements table
$buttons = Buttons::new()
    ->addButton(tr('Create'), DisplayMode::primary, '/phoundation/file-system/requirements/requirement.html')
...
$grid = Grid::new()
    ->addColumn($filters_card->render() . $requirements_card->render(), DisplaySize::nine)
