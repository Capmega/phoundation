            ...
            switch(isset_get($error[1])) {
                ...
                case 1049:
                    ...
                    if (($core->register['script'] == 'init')) {
                        if ($retry) {
                            $e = new CoreException(tr('sql_error(): Cannot use database ":db", it does not exist and cannot be created automatically with the current user ":user"', array(':db' => isset_get($query['db']), ':user' => isset_get($query['user']))), $e);
