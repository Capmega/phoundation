...
class socialmedia_oauth_connect
{
    ...
    public function Authorize()
    {
        ...
        if ($this->oauth_version == "2.0") {
            $dialog_url = $this->dialogUrl . "client_id=" . $this->client_id . "&response_type=" . $this->responseType . "&scope=" . $this->scope /*."&nonce=".$this->nonce*/ . "&state=" . $this->state . "&redirect_uri=" . urlencode($this->redirect_uri);
    ...
    public function curl_request($url, $method, $postvals)
    {
        ...
        if ($this->header) {
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, [$this->header . $postvals]);
        }
